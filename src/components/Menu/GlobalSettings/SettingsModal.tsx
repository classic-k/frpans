...
const SettingsModal: React.FC<React.PropsWithChildren<InjectedModalProps>> = ({ onDismiss, mode }) => {
  ...
  const [showExpertModeAcknowledgement, setShowExpertModeAcknowledgement] = useState(false)//useUserExpertModeAcknowledgementShow()
  const [expertMode, toggleExpertMode] = useState(false)//useExpertModeManager()
  const [singleHopOnly, setSingleHopOnly] =useState(false) //useUserSingleHopOnly()
  const [audioPlay, toggleSetAudioMode] = useState(false)//useAudioModeManager()
  const [zapMode, toggleZapMode] = useState()//useZapModeManager()
  const [subgraphHealth, setSubgraphHealth] = useState()//useSubgraphHealthIndicatorManager()
  const [userUsernameVisibility, setUserUsernameVisibility] = useState()//useUserUsernameVisibility()
  //const { onChangeRecipient } = useSwapActionHandlers()
  const onChangeRecipient = () => {
    return
  }
  const { chainId } = useActiveChainId()
  const [isStableSwapByDefault, setIsStableSwapByDefault] = useState(false)//useStableSwapByDefault()
